...
| `fct_weight_life_span` | one row per breed | Metric-focused: weight, height, life span, `weight_per_inch_lbs`, `life_span_category` |
...
- **CD:** on merge to `main`, `dbt run --target prod` + `dbt test --target prod` deploys models to the `gold` dataset.
